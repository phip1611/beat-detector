...
mod tests {
    ...
    fn test_f32_sample_to_i16() {
        ...
        unsafe {
            check!(f32_sample_to_i16_unchecked(F32Sample::try_from(0.0).unwrap()) == 0);
            check!(
                f32_sample_to_i16_unchecked(F32Sample::try_from(-0.5).unwrap()) == -i16::MAX / 2
            );
            check!(f32_sample_to_i16_unchecked(F32Sample::try_from(0.5).unwrap()) == i16::MAX / 2);
            check!(f32_sample_to_i16_unchecked(F32Sample::try_from(-1.0).unwrap()) == -i16::MAX);
            check!(f32_sample_to_i16_unchecked(F32Sample::try_from(1.0).unwrap()) == i16::MAX);
        }
    }
}
